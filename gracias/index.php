		        			...
		        			echo '<h1 class="display-4 justify-content-center align-items-center"><i class="fas fa-check-circle fa-1x text-success"></i> Tu pago ha sido procesado</h1>
								  ...
								  <p class="lead">
								    <a class="btn btn-primary btn-lg" href="../" role="button">Regresar</a>
								  	...
								  	$sqla = "SELECT a.street, a.number, a.cp, a.colony, a.city, a.state, c.name 
                                      FROM adresses as a 
                                      INNER JOIN orders as b on b.id_adress=a.id_adresses 
                                      INNER JOIN users as c on c.email=a.email_user
                                      WHERE b.id_orders=".$cart."";
...
function sendmailPayment($addReplyToEmail, $name, $street, $number, $cp, $colony, $city, $state, $numPedido, $result){
