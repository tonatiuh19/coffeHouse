 ...
 //
    ...
    // output data of each row
		echo ("<SCRIPT LANGUAGE='JavaScript'>
			window.alert('¡Este usuario ya existe! prueba con otro correo')
			window.location.href='../';
			</SCRIPT>");
	} else {
		$sql = "INSERT INTO users (email, name, last_name, type, date)
		VALUES ('$mail_i', '$name', '$lastname', '2', '$today')";

		if ($conn->query($sql) === TRUE) {
			$sql2 = "INSERT INTO new_supplier_messages (email_user, message)
			VALUES ('$mail_i', '$txt')";
			...
			if ($conn->query($sql2) === TRUE) {
				...
				echo ("<SCRIPT LANGUAGE='JavaScript'>
					window.alert('Enseguida un asesor se pondra en contacto contigo para darle seguimiento a tu proceso.')
					window.location.href='../../';
					</SCRIPT>");
			} else {
